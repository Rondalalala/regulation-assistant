...
function scrollToHighlight(id, delay = 0) {
  ...
  highlight.value = idStr

  // 若目标被当前筛选条件过滤掉了，先重置筛选确保它出现在 DOM 中
  const targetVisible = filtered.value.some(i => String(i.key) === idStr)
  if (!targetVisible) {
    filterDomain.value = ''
    filterCat.value = ''
  ...
  // 等待 transition 动画结束后再找 DOM（delay > 0 时跳过 nextTick 直接用 setTimeout）
  function doScroll() {
    function tryScroll(attempt = 0) {
      const el = document.getElementById(`auth-${idStr}`)
      if (el) {
        // 直接操作滚动容器而非 scrollIntoView，避免 Electron/transform 兼容问题
        const mainEl = document.querySelector('.main')
        if (mainEl) {
          const mainRect = mainEl.getBoundingClientRect()
          const elRect = el.getBoundingClientRect()
          const offset = elRect.top - mainRect.top + mainEl.scrollTop - mainEl.clientHeight / 3
...
onBeforeRouteUpdate((to) => {
  if (to.query.id) scrollToHighlight(to.query.id)
...
function matchSearch(item, kw) {
  ...
  // 3. bi-gram fallback（仅针对名称，阈值 0.6 减少误匹配）
  if (bigramScore(kw, name) >= 0.6) return true
