...
const defaults = {
  ...
  embedModel: 'jiaorong-qwen3-embedding-8b',
  embedBaseUrl: '',
  embedApiKey: '',
}
...
function save(s) {
  localStorage.setItem(STORAGE_KEY, JSON.stringify({
    ...
    embedApiKey: s.embedApiKey ? btoa(s.embedApiKey) : '',
  }))
}
...
if (state.embedApiKey && !state.embedApiKey.startsWith('sk-')) {
  try { state.embedApiKey = atob(state.embedApiKey) } catch { /* already plaintext */ }
}
...
export function useSettings() {
  ...
  function setEmbedModel(m) { state.embedModel = m; save(state) }
  function setEmbedBaseUrl(url) { state.embedBaseUrl = url; save(state) }
  function setEmbedApiKey(key) { state.embedApiKey = key; save(state) }
  ...
  return { settings: state, setApiKey, setBaseUrl, setModel, setEmbedModel, setEmbedBaseUrl, setEmbedApiKey, isConfigured, testConnection }
}
