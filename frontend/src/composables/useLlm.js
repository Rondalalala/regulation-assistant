...
export async function embedBatch(texts) {
  ...
  const baseUrl = settings.embedBaseUrl || settings.baseUrl
  const apiKey = settings.embedApiKey || settings.apiKey
  const model = settings.embedModel || 'jiaorong-qwen3-embedding-8b'

  const { headers } = buildProxyHeaders(baseUrl, {
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${apiKey}`,
    'User-Agent': 'XBTZ-RegulationPlatform/2.0',
  })

  const url = resolveUrl(baseUrl, 'embeddings')
  ...
  if (!res.ok) {
    ...
    const err = new Error(`Embedding Error ${res.status}: ${body.slice(0, 200)}`)
    err.status = res.status
    throw err
  }
